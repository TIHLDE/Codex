...
const validationSchema = yup.object().shape({
  ...
  start_registration_at: yup
    .date()
    .required('Du må fylle inn startdato for påmelding'),
  end_registration_at: yup
    .date()
    .required('Du må fylle inn sluttdato for påmelding'),
  location: yup.string().required('Du må fylle inn lokasjon'),
  mazemap_link: yup
    .string()
    .url('Må være en gyldig URL')
    .required('Du må fylle inn Mazemap-link'),
...
export const EventForm = ({ event }: EventFormProps) => {
  ...
  return (
    <form
      className="mx-auto w-full max-w-6xl space-y-8 rounded-md border border-gray-300 px-12 py-6 dark:border-slate-600"
    ...
    >
      ...
      <div className="flex space-x-8">
        <GroupDropdown
          ...
          onChange={(organizer) =>
            formik.setFieldValue('organizer', organizer, true)
          }
      ...
      <div className="space-y-2">
        ...
        {event && (
          <EventDeleteButton className="w-full" event={event} token={token} />
        )}
      </div>
    </form>
  );
