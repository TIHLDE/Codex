...
- TLS-sertifikater er konfigurert for `tihlde.org`. Se lengre ned for mer info om sertifikathåndtering.
...
## TLS-sertifikater / acme.sh

Drift bruker **acme.sh** for automatisk oppsett av TLS-sertifikater fra Let's Encrypt. Sertifikatene er lagret i `/etc/nginx/certificates/` og Nginx-konfigurasjonen peker til disse filene for SSL:
...
Som vi ser ligger sertifikater for _tihlde.org_ under `/etc/nginx/certificates/tihlde.org/`. Dette sertifikatet dekker også alle subdomener (`*.tihlde.org`) som vi bruker for våre tjenester.

### Installering av acme.sh på instanser

Dersom du trenger å installere _acme.sh_ på en annen instans følger du først installerings dokumentasjonen i [acme.sh sin README.md](https://github.com/acmesh-official/acme.sh).
...
Kjør så følgende kommando for å endre default sertifikat autoritet til Let's Encrypt:

```bash
acme.sh --set-default-ca --server letsencrypt
```

Så kjører du følgende for å skaffe et nytt sertifikat for `mysubdomain.tihlde.org` og alle dens subdomener:

```bash
DOMENESHOP_Token='<TOKEN>' DOMENESHOP_Secret='<SECRET>' acme.sh --issue -d mysubdomain.tihlde.org -d *.mysubdomain.tihlde.org --dns dns_domeneshop
```

Domeneshop token og secret blir lagret til senere bruk i `.acme.sh/account.conf`.

Så kan du installere sertifikatet til en destinasjon med:

```bash
acme.sh --install-cert -d mysubdomain.tihlde.org --fullchain-file /path/to/save/fullchain.pem --key-file /path/to/save/privkey.pem --reloadcmd "sudo systemctl reload nginx"
```

Dersom alt er good, vil dette installere sertifikatet og sette opp en cron-jobb for automatisk fornyelse. Når sertifikatet fornyes vil `sudo systemctl reload nginx` kjøres for å laste inn det nye sertifikatet.
